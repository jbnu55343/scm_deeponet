...
def parse_args():
    p = argparse.ArgumentParser("Build dataset with time-lag features from edgedata.xml")
    ...
    p.add_argument("--lag_features", nargs="+", default=[],
                   help="需要做滞后的特征子集（必须包含在 --features 里）")
    p.add_argument("--lags", nargs="+", type=int, default=[1,2],
...
def read_edgedata_xml(path_xml: str, features):
    """读取单个 edgedata.xml，返回：
       times: (T,)  每个区间的 begin（或中点）
       edges: list[str]  边 ID 列表
       X_tef: (T, E, F)  三维数组，缺失填 0
    """
    tree = ET.parse(path_xml)
    root = tree.getroot()
    intervals = [iv for iv in root.findall("interval")]
    if not intervals:
        return None
...
def build_one_scenario(sdir: Path, features, lag_features, lags, target, horizon):
    ...
    # 生成滞后特征
    lag_names = []
    lag_arrays = []  # list of (T,E, #lag_features*len(lags))
    ...
    yv = X_tef[valid_t_start+horizon:valid_t_end+horizon, :, target_idx]  # (Tv,E)
...
def main():
    ...
    for d in dirs:
        r = build_one_scenario(d, args.features, args.lag_features, args.lags, args.target, args.horizon)
        if r is None:
            print(f"[SKIP] {d} (no edgedata or too few intervals)")
            continue
        Xf, Yf, feat_names, meta, edges = r
        X_list.append(Xf); Y_list.append(Yf); metas.append(meta); edges_map[meta["scenario"]] = edges
        print(f"[OK] {d.name}: Tv={meta['Tv']}, E={meta['E']}, D={len(feat_names)}, samples={len(Xf)}")
    ...
    split = {"train": idx[:n_tr], "val": idx[n_tr:n_tr+n_va], "test": idx[n_tr+n_va:]}
