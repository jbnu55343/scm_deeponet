...
class NpzDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        y = self.Y[j].astype(np.float32, copy=False)

        # Ensure y is an array (if it's a scalar)
        if y.ndim == 0:
            y = y.reshape(1)
...
def main():
    ...
    feat_str = str(feats) if len(feats) < 20 else f"[{len(feats)} features]"
    print(f"[LOAD] X={X.shape} Y={Y.shape} features={feat_str}")
    ...
    if args.filter_traveltime:
        ...
        # If filtering, we might invalidate the pre-defined split indices if they refer to original indices
        # For simplicity, if filtering is on, we might fall back to random split or need complex re-indexing.
        # Assuming filter_traveltime is False for METR-LA.
    else:
        keep = np.arange(X.shape[0], dtype=np.int64)

    # Use pre-defined split if available and no filtering (or filtering didn't drop anything)
    if split is not None and not args.filter_traveltime:
        print("[SPLIT] Using pre-defined split from NPZ file")
        train_idx = np.array(split['train'])
        val_idx = np.array(split['val'])
        test_idx = np.array(split['test'])
    else:
        print("[SPLIT] Using random split")
        # 简单随机划分
        N = keep.size
        idx = keep.copy()
    ...
    # Check for original stats in metadata
    orig_std_val = 1.0
    if 'norm_std' in meta:
        orig_std_val = float(meta['norm_std'])
        print(f"[METADATA] Found original std: {orig_std_val:.2f} (for restoring real units)")
    ...
    for epoch in range(1, args.epochs+1):
        ...
        print(f"[E{epoch:02d}] train_mse={np.mean(losses):.4f}  "
              f"val: MAE={val_mae:.3f} (Real={val_mae*orig_std_val:.1f}) "
              f"RMSE={val_rmse:.3f} (Real={val_rmse*orig_std_val:.1f}) "
              f"R2={val_r2:.4f}  ({dt:.1f}s)")
    ...
    # Real values
    real_mae = test_mae * orig_std_val
    real_rmse = test_rmse * orig_std_val
    
    print(f"[TEST] MAE={test_mae:.3f} (Real={real_mae:.1f}) "
          f"RMSE={test_rmse:.3f} (Real={real_rmse:.1f}) "
          f"R2={test_r2:.4f}")
    ...
    result = {
        ...
        "test_mae": float(real_mae),
        "test_rmse": float(real_rmse),
