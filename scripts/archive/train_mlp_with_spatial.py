#!/usr/bin/env python3
"""
MLP training on SPATIAL dataset (with spatial features from neighboring edges).
...
MODEL_SAVE_PATH = "models/mlp_spatial.pt"
RESULTS_SAVE_PATH = "results/mlp_spatial_results.npz"
...
def evaluate(model, loader, device):
    ...
    with torch.no_grad():
        for X_batch, y_batch in loader:
            ...
            # For R², accumulate components
            ss_res_list.append(((y_true - y_pred) ** 2).sum())
            
            # Running mean for y_true
    ...
    # For R² - need to recompute with correct mean
    # Do a second pass to get correct mean
    y_mean_total = 0.0
    n_total = 0
    with torch.no_grad():
        for X_batch, y_batch in loader:
    ...
    # Now compute R² with correct mean
    ss_res = 0.0
    ss_tot = 0.0
    with torch.no_grad():
        for X_batch, y_batch in loader:
...
def main():
    print("="*80)
    print("MLP Training: Spatial Dataset")
    print("="*80)
    ...
    model = MLPSpatial(input_size=input_size, hidden_size=256, dropout=0.1).to(DEVICE)
